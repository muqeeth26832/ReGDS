...
public:
    ...
    // std::vector<typename boost::graph_traits<Graph>::vertex_descriptor> vf2pp_vertex_order(const Graph &g)
    // {
    ...
    template <typename Graph>
    std::vector<unsigned long int> vf2pp_vertex_order(const Graph &g)
    {
        // using Vertex = typename boost::graph_traits<Graph>::vertex_descriptor;
        typename boost::graph_traits<Graph>::vertex_iterator VertexIterator;
        ...
        for (; vi != vi_end; ++vi)
        {
            order.push_back(static_cast<unsigned long int>(*vi));
        }

        // sort vertices in des order
        std::sort(order.begin(), order.end(), [&g](unsigned long int a, unsigned long int b)
                  { return degree(static_cast<Vertex>(a), g) > degree(static_cast<Vertex>(b), g); });
        return order;
    }
